package ma.enset.digital-bankingbackend.web;
...
import ma.enset.digital-bankingbackend.services.ActionLogServiceImpl;
import ma.enset.digital-bankingbackend.services.BankAccountService;
...
public class BankAccountRestController {
    ...
    public DebitDTO debit(@RequestBody DebitDTO debitDTO, Principal principal)
            throws BankAccountNotFoundException, BalanceNotSufficientException {
        bankAccountService.debit(debitDTO.getAccountId(), debitDTO.getAmount(), debitDTO.getDescription());
        actionLogServiceImpl.log(principal.getName(), "DEBIT",
                "Debited " + debitDTO.getAmount() + " from account " + debitDTO.getAccountId());
        return debitDTO;
    }

    @PostMapping("accounts/credit")
    public CreditDTO credit(@RequestBody CreditDTO creditDTO, Principal principal)
            throws BankAccountNotFoundException, BalanceNotSufficientException {
        bankAccountService.credit(creditDTO.getAccountId(), creditDTO.getAmount(), creditDTO.getDescription());
        actionLogServiceImpl.log(principal.getName(), "CREDIT",
                "Credited " + creditDTO.getAmount() + " to account " + creditDTO.getAccountId());
        return creditDTO;
    }

    @PostMapping("/accounts/transfer")
    public void transfer(@RequestBody TransferDTO transferDTO, Principal principal)
            throws BankAccountNotFoundException, BalanceNotSufficientException {
        bankAccountService.transfer(
                transferDTO.getAccountSource(),
                transferDTO.getAccountDestination(),
                transferDTO.getAmount());
        actionLogServiceImpl.log(principal.getName(), "TRANSFER", "Transferred " + transferDTO.getAmount()
                + " from account " + transferDTO.getAccountSource() + " to " + transferDTO.getAccountDestination());
    }

    @PostMapping("/accounts")
    public BankAccountDTO create(@RequestBody BankAccountRequestDTO bankAccountRequestDTO, Principal principal)
        ...
        actionLogServiceImpl.log(principal.getName(), "CREATE_ACCOUNT",
                "Created " + bankAccountRequestDTO.getType() + " account with ID " + id);
        return result;
    }
    ...
    public ResponseEntity<?> updateStatus(
        ...
        actionLogServiceImpl.log(principal.getName(), "UPDATE_ACCOUNT_STATUS",
                "Updated status of account " + id + " to " + newStatus);
        return ResponseEntity.ok("Status updated to " + newStatus);
    }

    @PutMapping("/accounts/{id}/interest-rate")
    @PreAuthorize("hasRole('USER')")
    public void updateInterestRate(@PathVariable String id, @RequestBody double newRate)
    ...
    @PreAuthorize("hasRole('USER')")
    public void updateOverdraft(@PathVariable String id, @RequestBody double newLimit)
            throws BankAccountNotFoundException {
    ...
    public ResponseEntity<?> cancelOperation(@PathVariable String id, Principal principal)
            throws BankAccountNotFoundException {
